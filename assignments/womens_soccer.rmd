...
```{r}
maryland_corners <- matches_2022 %>% 
  filter(team == "Maryland Terrapins, Terps") %>%
  summarize(total_corners = sum(corners))

md_opponent_corners <- matches_2022 %>%
  filter(team == "Maryland Terrapins, Terps") %>%
  group_by(opponent) %>%
  summarize(total_opponent_corners = sum(defensive_corners))

total_md_opponent_corners <- md_opponent_corners %>%
  summarize(total_corners = sum(total_opponent_corners))

corner_differential_per_game <- 
  mutate(percent_quality_shots = (corner_kick_differential = total_opponent_corners-corners))
