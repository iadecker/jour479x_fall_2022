...
```{r}
logs <- read_csv("http://dwillis.github.io/sports-data-files/wbb_game_logs_2022.csv")

logs %>% summarise(correlation = cor(win, pf, method="pearson"))

fit <- lm(win ~ pf, data = logs)
summary(fit)

newcorrelations <- logs %>% 
...
```{r}
fouls_by_month <- unique_games %>%
  mutate(date = mdy(date)) %>%
  group_by(month(date)) %>%
  summarise(monthly_total = sum(home_fouls) + sum(visitor_fouls), games = n()) %>%
...
fouls_by_month_and_team <- unique_games %>%
  filter(home %in% big_10) %>%
  mutate(date = mdy(date)) %>%
  group_by(month(date), home) %>%
  summarise(monthly_total = sum(home_fouls) + sum(visitor_fouls), games = n()) %>%
  mutate(avg_fouls_per_month = monthly_total/games)

fouls_by_month_and_team_visitor <- unique_games %>%
  filter(visitor %in% big_10) %>%
  mutate(date = mdy(date)) %>%
  group_by(month(date), visitor) %>%
  summarise(monthly_total = sum(home_fouls) + sum(visitor_fouls), games = n()) %>%
  mutate(avg_fouls_per_month = monthly_total/games)

all_fouls_by_month_and_team <- unique_games %>%
  mutate(date = mdy(date)) %>%
  group_by(month(date), home) %>%
  summarise(monthly_total = sum(home_fouls) + sum(visitor_fouls), games = n()) %>%
  mutate(avg_fouls_per_month = monthly_total/games)

all_fouls_by_month_and_team_visitor <- games_with_refs %>%
  mutate(date = mdy(date)) %>%
  group_by(month(date), visitor) %>%
  summarise(monthly_total = sum(total_fouls), games = n()) %>%
  mutate(avg_fouls_per_month = monthly_total/games)
```
