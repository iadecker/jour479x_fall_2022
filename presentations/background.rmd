...
departures <- anti_join(nats_roster_aug1, nats_roster_aug10, by = person_full_name, copy = FALSE, na_matches = c("na", "never"))

```



Summarise the gap in production of who is out and in, and then do that for other roster blowups, and try to wuantify the hole they put themselves in? How long did it take them to climb out of it? Can we see how long it takes?

anti-join on player_id
after anti join to before based on player id
